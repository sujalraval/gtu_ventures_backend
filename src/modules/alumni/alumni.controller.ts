...
function buildDirectoryEntry(user: StartupDataUser): DirectoryEntry {
  ...
  return {
    ...
    description: app?.briefAbout || 'A startup from the GTU Incubation Program.',
        ...
        const results = await Promise.allSettled(
          users.map(u =>
            sendEmail(
              ...
              `<p>Dear ${u.name},</p><p>Please submit your annual KPI snapshot for <strong>${snapshotYear}</strong> on the GTU Alumni Portal.</p>`
