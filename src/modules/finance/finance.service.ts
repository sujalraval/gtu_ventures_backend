...
export class FinanceService {
  ...
  static async getLedger(filters?: { accountId?: string; search?: string }) {
    try {
      ...
      // Map back to GTU's frontend expected Voucher flat-structure
  ...
  static async getStartupGrantMilestones() {
    try {
      // Query GTU's existing tranches database joined to startups, grants, and milestones
      ...
      // Filter and map GTU's core grant items into flat UI rows
  ...
  static async payoutGrantTranche(trancheId: string, utr?: string) {
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        // 4. Update the existing StartupGrantTranche status in GTU database
